...
//
//  CCritSec - Critical Section (MT synchronization within the same process)
//

#if defined(HNX_SINGLE_THREADED)
// single threading version
#include "critsec_st.hpp"

#else // multi-threading on multi-processors / uni-processor

#if defined(_WINBASE_)
#include "critsec_win32.hpp"
#elif defined(_PTHREAD_H) || defined(PTHREAD_H)
#include "critsec_pthread.hpp"
#else
#if defined(_MSC_VER)
#pragma message (__FILE__ ": configuration warning: Critical Section is not supported by the unknown platform.")
#else
#warning "configuration warning: Critical Section is not supported by the unknown platform."
#endif
