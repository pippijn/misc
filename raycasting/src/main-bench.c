...
rdtsc (void)
{
  unsigned int high = 0;
  unsigned int low = 0;
  __asm__ volatile ("rdtsc" : "=d" (high), "=a" (low));
...
#define TIME_END                                        \
      ...
      cycles = min (cycles, end - start);               \
    }                                                   \
  return cycles

static cycles_t
min (cycles_t a, cycles_t b)
{
  return a < b ? a : b;
}

static cycles_t
bench1 (rc_ctx const *ctx, long px, long py, size_t max)
{
  TIME_START {
    (void)rc_contains (ctx, px, py);
  } TIME_END;
}

static void
bench (rc_ctx const *ctx, long px, long py)
{
  int i;
  cycles_t cycles = ULONG_MAX;
  for (i = 0; i < 5; i++)
    {
      cycles = min (bench1 (ctx, px, py, (size_t)1000), cycles);
      printf ("%d: %lu cycles\n", i, cycles);
    }
}
#endif
